$(function () {
  ...
  $('#add-post').on('click', function () {
    var open_url = 'popup_posts.php?popup=1';
  ...
  $.related_links.fncall.add_link = function () {
    ...
    if (!in_array && data.id != $('#id').val()) {
      ...
      if (links.length > 0) {
        position =
          $('#related-links-list li.link:last input[type=hidden]').val() + 1;
      } else {
      ...
      var li = $(
        '<li class="link"><input type="hidden" name="related_link_rank[' +
          data.id +
          ']" value="' +
          position +
          '"/><a class="post-' +
  ...
  $('#related-links-list').sortable({
    ...
    update: function () {
      $(this)
        ...
        .each(function (i) {
          $(this)
            .find('input[name^=related_link_rank]')
            .each(function () {
              $(this).attr('value', (i + 1) * 1);
            });
        });
    },
  });

  $('.related-links-area label').toggleWithLegend(
    $('.related-links-area').children().not('label'),
    {
      user_pref: 'dcx_related_links',
      legend_click: true,
    }
  );
});
